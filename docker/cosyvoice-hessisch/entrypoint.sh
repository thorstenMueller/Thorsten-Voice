...
MODEL_DIR="/app/CosyVoice/pretrained_models/CosyVoice3-Hessisch"
BASE_MODEL="FunAudioLLM/Fun-CosyVoice3-0.5B-2512"
THORSTEN_MODEL="Thorsten-Voice/CosyVoice3-Hessisch"

echo "========================================="
echo "  Thorsten-Voice CosyVoice3 TTS Server"
echo "  Hessisch Edition"
echo "========================================="
...
# Download Hessisch fine-tuned weights if any of the three files is missing
if [ ! -f "$MODEL_DIR/llm.pt" ] || \
   [ ! -f "$MODEL_DIR/flow.pt" ] || \
   [ ! -f "$MODEL_DIR/spk2info.pt" ]; then
    echo "[2/2] Downloading Thorsten-Voice Hessisch fine-tuned weights (~3.2 GB)..."
    hf download "$THORSTEN_MODEL" \
        --local-dir "$MODEL_DIR" \
        --include "llm.pt" "flow.pt" "spk2info.pt"
    echo "Hessisch weights downloaded ✓"
else
    echo "[2/2] Hessisch weights already present ✓"
fi
